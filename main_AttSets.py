...
#		tf.summary.image(name="2D_MaxP",tensor=l2,max_outputs=1)
		...
		l12 = tf.reshape(l12, [-1, int(d1) * int(d2) * int(cc)],name="r2n_fc_in")
		...
		#### use fc attention
		input = tf.reshape(fc, [-1, im_num, 1024],name="r2n_att_in")
		...
		latent_3d = tf.reshape(latent_3d, [-1, 4, 4, 4, 128],name="r2n_att_out")
		...
		y = tf.nn.sigmoid(d11,name='r2n_sigmoid')

		xy = tf.reshape(y, [-1, vox_res32, vox_res32, vox_res32],name="r2n_out")
